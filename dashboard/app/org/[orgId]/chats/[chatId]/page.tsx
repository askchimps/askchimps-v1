...
export default function ChatDetailPage() {
    ...
    const initialChatId = params.chatId as string;
    ...
    useEffect(() => {
        ...
        if (statusParam) setStatus(statusParam as ChatStatus);
        if (sourceParam) setSource(sourceParam as ChatSource);
    }, [searchParams]);
    ...
    const isChatInCurrentPage = chats.some(
        (chat) => chat.id === selectedChatId,
    );

    // Scroll to the selected chat on page load (if it's in the list)
    useEffect(() => {
        if (isChatInCurrentPage && selectedChatId && !isLoadingChats) {
    ...
    const buildUrl = (
        ...
        const targetChatId = newChatId || selectedChatId;
        return `/org/${orgId}/chats/${targetChatId}${queryString ? `?${queryString}` : ""}`;
    };

    // Update URL when chat is selected (without full navigation)
    const handleChatSelect = (newChatId: string) => {
        // Update local state immediately (no re-render jump)
        setSelectedChatId(newChatId);

        // Update URL without triggering navigation
        const url = buildUrl(newChatId);
        if (url) {
