...
export class CreateChatDto {
    ...
    @ApiProperty({
        description:
            'Unique identifier from the source platform (e.g., WhatsApp phone number, Instagram user ID)',
        example: '+1234567890',
        maxLength: 255,
    })
    @IsNotEmpty()
    ...
    @ApiPropertyOptional({
        description:
            'Detailed summary of the chat conversation (AI-generated or manual)',
        example:
            'Customer John Doe contacted via WhatsApp asking about solar panel installation. Interested in 5kW system for residential property. Requested quote and installation timeline.',
    })
    @IsOptional()
    @IsString()
    ...
    @ApiPropertyOptional({
        description:
            'Whether the chat has been transferred to another agent/system',
        example: false,
        default: false,
    })
    @IsOptional()
    ...
    @ApiPropertyOptional({
        description:
            'Array of tag IDs to associate with this chat (ULID format)',
